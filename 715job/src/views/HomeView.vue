<template>
  <div>
    ...
    <div ref="MainCalendar">
      <MainCalendar
        v-if="isBooking"
        @isDatePicked="showFloorPlan"
        :hideDummy="isDatePicked"
      />
    </div>
    <div ref="MainFloorPlan">
      <MainFloorPlan
        v-if="isDatePicked"
        @isTablePicked="showRsvContents"
        :hideDummy="isTablePicked"
      />
    </div>
...
export default {
  ...
  watch: {
    isTablePicked() {
      window.scrollTo({
        top: this.$refs.MainRsvContents.offsetTop + 96,
        behavior: 'smooth'
      })
    }
  },
  methods: {
    setInput(isBooking) {
      ...
      window.scrollTo({
        top: this.$refs.MainCalendar.offsetTop,
        behavior: 'smooth'
      })
    },
    showFloorPlan(isDatePicked, PickedDate) {
      ...
      window.scrollTo({
        top: this.$refs.MainFloorPlan.offsetTop + 96,
        behavior: 'smooth'
      })
    },
    showRsvContents(isTablePicked, PickedTable) {
      ...
      window.scrollTo({
        top: this.$refs.MainFloorPlan.offsetTop + 134,
        behavior: 'smooth'
      })
    }
  }
